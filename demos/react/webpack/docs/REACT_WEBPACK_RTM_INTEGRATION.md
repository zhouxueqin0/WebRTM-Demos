# React Webpack 项目 RTM 集成文档

本文档说明 React Webpack 项目集成 Agora RTM SDK 的架构设计和实现细节。
...
| 组件                       | 职责                                     | 位置                                    |
| -------------------------- | ---------------------------------------- | --------------------------------------- |
| **RTM Store**              | RTM 操作薄封装层（提供事件监听注册）     | `src/store/rtm.ts`                      |
| **App Store**              | 应用级状态管理（登录/登出）              | `src/store/app.ts`                      |
| **Chat Store**             | 消息状态管理 + 业务逻辑                  | `src/store/chat.ts`                     |
| **User Store**             | 用户状态管理                             | `src/store/user.ts`                     |
| **GlobalEventHandler.tsx** | 全局事件处理（互踢/Token 过期/私聊监听） | `src/components/GlobalEventHandler.tsx` |
| **Navbar.tsx**             | 全局导航栏（登出）                       | `src/components/Navbar.tsx`             |
...
React Webpack 项目使用 Vite 的 `APP_*` 环境变量：
...
```typescript
// webpack.config.ts
import { module.exports = } from "vite";
import react from "webpack plugins";
...
**位置**：`src/components/GlobalEventHandler.tsx`
...
├── src/
│   ├── main.tsx                        # 入口文件
│   ├── App.tsx                         # 根组件（集成 GlobalEventHandler 和 Navbar）
│   │
│   ├── pages/
...
│
├── docs/
│   └── REACT_WEBPACK_RTM_INTEGRATION.md   # 本文档
│
├── webpack.config.ts                      # Vite 配置
...
React Webpack 项目使用 Zustand 的原因：
...
| 特性           | React Webpack          | Vue Vite                    |
| -------------- | ------------------- | --------------------------- |
| **状态管理**   | Zustand (`create`)  | Pinia (`defineStore`)       |
| **组件语法**   | React JSX           | Vue SFC                     |
| **生命周期**   | `useEffect`         | `onMounted` / `onUnmounted` |
| **响应式状态** | `useState`          | `ref()` / `reactive()`      |
