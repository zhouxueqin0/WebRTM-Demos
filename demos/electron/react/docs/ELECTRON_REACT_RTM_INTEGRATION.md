# Electron React 项目 RTM 集成文档

本文档说明 Electron React 项目集成 Agora RTM SDK 的架构设计和实现细节
...
├── src/
│   ├── App.tsx                     # 主应用（集成 GlobalEventHandler 和 Navbar）
│   ├── pages/
│   │   ├── Login.tsx               # 登录页
│   │   ├── Home.tsx                # Home 页面
...
| **User Store**             | 用户状态管理                             | `store/user.ts`                     |
...
export default function GlobalEventHandler() {
  useEffect(() => {
    // 登录后，全局监听私聊消息
    useChatStore.getState().registerPrivateMessageListener();

    return () => {
      useChatStore.getState().unregisterPrivateMessageListener();
    };
  }, []);
...
export default function GlobalEventHandler() {
  ...
  const handleLinkState = useMemo(() => {
    return async (eventData: RTMEvents.LinkStateEvent) => {
      ...
      if (currentState === "FAILED" && reasonCode === "TOKEN_EXPIRED") {
        await useRtmStore.getState().rtmLogin();
      }
    };
  }, []);
  ...
  useEffect(() => {
    ...
    useRtmStore.getState().registerLinkStateListener(handleLinkState);

    return () => {
      useChatStore.getState().unregisterPrivateMessageListener();
      useRtmStore.getState().unRegisterLinkStateListener(handleLinkState);
    };
  }, [handleLinkState]);
...
const handleLinkState = useCallback(
  async (eventData: RTMEvents.LinkStateEvent) => {
...
Electron React 使用 `react-router-dom` 进行路由管理：
...
export default function Message() {
  ...
  return (
    <div className="message-container">
      <ClassroomList onClassroomClick={handleClassroomClick} />
      <TeacherList onTeacherClick={handlePrivateChatClick} />
      <ChatDrawer
        state={drawerState}
